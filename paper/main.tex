...
\label{sec:e3}
To confirm the abstraction does not cost accuracy, we train the \emph{same}
recipe --- ResNet-18 from scratch, $32{\times}32$, five epochs, AdamW at
$3{\times}10^{-4}$ with cosine annealing, AMP, and identical resize+horizontal-flip
augmentation --- with a hand-written loop and with FujiCV, and compare top-1
validation accuracy on CIFAR-10 (Table~\ref{tab:accuracy}). Matching the recipe
is essential: an initial FujiCV run that inadvertently used a stronger
augmentation preset underfit at this short budget and scored $66.2\%$, a gap
attributable to the augmentation policy, not the framework; with augmentation
equalized the two agree within run-to-run variance.

\begin{table}[h]
\centering
\caption{Top-1 CIFAR-10 validation accuracy (\%), identical from-scratch recipe.
FujiCV matches the hand-written loop.}
\label{tab:accuracy}
\small
\begin{tabular}{lccc}
\toprule
...
\midrule
CIFAR-10 (ResNet-18, 5 ep, scratch) & 71.9 & \textit{PENDING} & \textit{PENDING} \\
\bottomrule
\end{tabular}
\end{table}
